    ...
    public void handlePacket(ClientPlayNetworking.Context context) {
        ...
        if (!(entity instanceof NPCEntity npc)) {
            CiviliansMod.LOGGER.warn(
                    "[CiviliansMod] Dialogue packet received but entity {} is not NPCEntity (was {})",
                    npcId,
        ...
        Map<String, Map<NpcChat.ChatReason, List<String>>> dialogueMap =
                new Gson().fromJson(dialogue, type);

        npc.getChatManager().setDialogues(dialogueMap);
        npc.dialoguesReceived = true;

        CiviliansMod.LOGGER.info(
                "[CiviliansMod] Dialogues received for NPC {} (dialoguesReceived=true)",
                npcId
        );
        ...
        MinecraftClient.getInstance().execute(() -> {
            if (MinecraftClient.getInstance().currentScreen instanceof CustomChatScreen screen) {
                CiviliansMod.LOGGER.info(
                        "[CiviliansMod] Initializing CustomChatScreen after dialogue sync for NPC {}",
                        npcId
                );
                screen.fullInit();
            } else {
                CiviliansMod.LOGGER.info(
                        "[CiviliansMod] Dialogues received but screen not open yet for NPC {}",
                        npcId
                );
            }
        });
